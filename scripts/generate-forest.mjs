...
const LEVEL_COLORS = ["#8a8a8a", "#a8d08a", "#6dae52", "#3f7a2f", "#1e3d14"];
...
function buildSVG(weeks) {
  ...
  <style>
    .tree {
      ...
      /* Sem opacity/transform aqui de propósito: se o visualizador não rodar
         a animação (ex.: alguns apps mobile), a árvore continua VISÍVEL no
         estado padrão do SVG, em vez de ficar presa em "invisível". A
         animação abaixo é só um efeito extra, não uma dependência. */
    ...
    @keyframes sprout {
      ...
      94%  { opacity: 0.85; transform: scale(0.95); }
